...
Map
	...
	만약에 중복된 키가 존재한다면 이전 키는 나중에 작성한 키로 대체됨
	순서가 보장되지 않음, 만약 순서를 보장해야하는 경우엔 LinkedHashMap을 사용
...
메서드
	...
	containsValue(value)	: 맵에 지정된 키가 포함되어 있는지 확인
	remove(key)				: 지정된 키와 해당하는 값을 제거
	
Map.Entry<key자료형, value자료형>
	Map 인터페이스 안에서 작성된 내부 클래스 Entry 의 키와 값의 쌍을 나타냄
	entrySet()	: 키-값에 대한 정보를 보여줌
				모든 키와 값의 쌍을 한번에 보여줌
	Map.Entry	: 전체 키-값 쌍에서 1쌍씩 꺼내서 보여줌
	getKey()	: 키를 가져옴
	getValue()	: 값을 가져옴
*/
public class Map {
